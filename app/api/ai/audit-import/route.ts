...
export async function POST(request: NextRequest) {
  try {
    ...
    const questions = template.fa_audit_template_sections
      ?.flatMap((section: any) => section.fa_audit_template_questions || [])
      .map((q: any) => ({
        id: q.id,
    ...
    questions.forEach((q) => {
      ...
      if (value) {
        seedTextAnswers[q.id] = cleanExtractedValue(value)
      }
    })
    ...
    const prompt = `
You are importing a legacy safety audit PDF into our SafeHub system.

Extract answers from the PDF text and map them to the template questions.
