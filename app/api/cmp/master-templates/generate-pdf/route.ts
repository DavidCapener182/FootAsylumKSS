...
async function renderTemplatePdf(
  ...
  const reportUrl = `${baseUrl}/print/cmp-master-template?${reportParams.toString()}`
...
function getSafeArchiveFilename() {
  return `cmp-master-templates-${new Date().toISOString().slice(0, 10)}.zip`
}
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const selectedTemplates = uniqueTemplateIds
      .map((templateId) => getCmpMasterTemplateById(templateId))
      .filter((template): template is CmpMasterTemplateDefinition => Boolean(template))
    ...
    console.error('Error generating CMP master template archive:', error)
    if (browser) {
      try {
        await browser.close()
      } catch {}
    ...
    return NextResponse.json(
      { error: 'Failed to generate CMP master template archive', details: error?.message || String(error) },
      { status: 500 }
    )
  }
}
