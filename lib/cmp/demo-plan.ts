import { CMP_ANNEX_DEFINITIONS } from '@/lib/cmp/master-template'

const lines = (...items: string[]) => items.join('\n')

export const CMP_DEMO_EVENT_NAME = 'Northgate Summer Live 2026'
export const CMP_DEMO_PLAN_TITLE = `Crowd Management and Security Operations Plan - ${CMP_DEMO_EVENT_NAME}`
export const CMP_DEMO_SELECTED_ANNEXES = CMP_ANNEX_DEFINITIONS.map((annex) => annex.key)

export const CMP_DEMO_PLAN_VALUES: Record<string, string> = {
  plan_title: CMP_DEMO_PLAN_TITLE,
  ...
  purpose_scope_summary:
    'This Crowd Management and Security Operations Plan defines the crowd safety, security, safeguarding, communications, emergency response, and post-event reporting arrangements for Northgate Summer Live 2026. The document applies to all public and controlled areas of the event site, including the arena, ingress and egress routes, campsite, bars, VIP compound, transport interfaces, and emergency rendezvous points. It is intended to be read alongside the Event Management Plan, operational risk assessment, licensing documentation, current site plans, and daily control briefings.',
  ...
  key_delivery_partners: lines(
    'KSS NW LTD - Security and crowd management',
  ...
  licensed_capacity: '18,000 public attendees in the arena, 4,500 persons in the campsite, and 22,500 maximum total site occupancy including staff and contractors.',
  ...
  attendance_profile:
    'The audience profile is primarily mainstream live-music festival attendees with a strong evening demand pattern around headline performances and licensed bar activity. Daytime attendance includes families and mixed-age groups, while the campsite introduces overnight welfare and vulnerability considerations. The venue is expected to attract a high proportion of attendees travelling by rail and shuttle bus, with moderate levels of alcohol consumption increasing into the evening.',
  travel_modes:
  ...
  site_layout_summary:
    'The site is arranged as a fenced outdoor arena with a main stage, second stage, food court, licensed bars, welfare and medical hub, accessible viewing platform, campsite to the north, VIP and backstage compound to the west, and dedicated ingress and egress routes on the east and south boundaries. Controlled service access is separated from public routes by barrier lines and staffed crossing points.',
  key_zones: lines(
    ...
    'North campsite and campsite hub',
    'VIP and backstage compound',
    'South and east egress corridors',
    'Transport interface and taxi marshal zone'
  ),
  controlled_areas:
    'Controlled areas comprise the backstage compound, production lane, VIP entry, artist accreditation zone, medical treatment facility, safeguarding interview room, cash handling point, plant and generator compound, and emergency vehicle lane. These areas are restricted to authorised badge holders or escort-controlled access.',
  emergency_exits_holding_areas:
    'Primary emergency exits are located at the south boulevard, east plaza, and north crossover to the outer park. Secondary exits exist at the west vehicle gate and campsite emergency release route. Holding areas are designated in the east overflow car park and north outer field. Rendezvous points are established at RV1 east service yard, RV2 north outer field, and RV3 south coach park.',
  dim_aliced_design:
    'The site design provides segregated ingress and egress, sufficient viewing depth in front of the main stage, step-free accessible routes, clearly defined sterile areas, and barrier-supported lane management at each public interface. Public information points and water distribution are positioned away from primary circulation routes to reduce dwell conflict.',
  ...
  gross_area: 'Main arena gross audience area: 38,400 m2. Total public site footprint including campsite and plazas: 92,000 m2.',
  ...
  density_assumptions:
    'Main stage fan has been assessed at a managed operational density up to 2 persons per m2 with intervention thresholds well below this in the front-of-stage line. General arena circulation areas are assessed at 1 person per m2 or lower. Queue and plaza calculations use conservative throughput assumptions to reflect search activity and variable luggage profile.',
  zone_capacities: lines(
    ...
    'North campsite occupancy cap - 4,500 persons'
  ),
  ...
  named_command_roles: lines(
    'Operational Lead - David Capener - Overall responsibility for crowd management and security delivery.',
    ...
    'Arena Supervisor - Sophie Grant - Main arena circulation, front-of-stage liaison, and west bar village.',
    'Campsite Supervisor - Reece Bolton - Campsite control, campsite welfare, and overnight response.',
  ...
  radio_channels_callsigns: lines(
    ...
    'Channel 3 - Arena and front-of-stage response.',
  ...
  external_interfaces: lines(
    ...
    'Open Arms Welfare supervisor linked to welfare hub and campsite safe space.',
  ...
  monitoring_and_density_tools:
    'Live monitoring combines fixed supervisor observation, front-of-stage reporting, CCTV review from Event Control, welfare and medical feedback, queue-marshals at ingress and bar zones, and structured patrol reporting from mobile response teams. The event does not rely on automated density systems for decision making, but any temporary people-counting or video analytics feed would be treated as supporting information only.',

  service_delivery_scope:
    'KSS is responsible for the east ingress search and admissions plaza, main arena circulation, west bar village security support, campsite security and welfare interface, VIP and backstage access control, south and east dispersal routes, and mobile response capability. Ticket scanning, primary traffic management, artist close protection, and medical treatment remain with the relevant specialist providers, but KSS maintains live operational interfaces with each function.',
  ...
  staffing_by_zone_and_time:
    lines(
      ...
      '22:30 to 00:30 - South boulevard and east plaza dispersal - 46 staff across route stewards, taxi marshals, shuttle loading, and campsite split.',
      '23:00 to 08:00 - Campsite and overnight welfare - 20 staff including campsite patrols, welfare support, and relief cover.'
    ),
  response_teams:
    lines(
  ...
  front_of_stage_roles:
    'Pit Team Supervisor controls barrier-line staffing, extraction points, and communication with production during headline sets. Front-of-stage responders are divided between stage-left and stage-right positions, with roaming high-density supervisors reporting crowd condition changes back to Event Control.',
  ...
  camping_security_roles:
    'Camping Security Supervisor manages overnight patrols, perimeter observation, welfare-linked patrol teams, campsite response, and campsite-hub access control. Quiet-hours interventions, fire-watch observations, and lost-person reunification in the campsite are coordinated from the campsite supervisor channel.',
  vip_backstage_roles:
    'VIP and Backstage Security Supervisor manages accreditation checks at the west compound gate, artist-route protection between compound and stage access, escort arrangements for restricted areas, and rapid escalation for unauthorised access or challenge to sterile routes.',
  stewarding_roles:
    'Stewarding roles include directional staff on the arena spine, queue marshals at ingress and bar feeder lanes, emergency-exit stewards, campsite wayfinding stewards, and route-clearance stewards during final egress. Steward brief ownership sits with the relevant zone supervisor and all stewarding changes are logged through Event Control.',
  ...
  safe_spaces:
    'Primary safe space provision is located within the welfare hub adjacent to the medical facility, with a secondary quiet room in the campsite hub overnight. Both locations provide private seating, water, radio contact with control, and direct handover to the welfare or safeguarding lead.',
  ...
  licensable_activities:
    'Licensable activities include the sale of alcohol, regulated entertainment across both stages, late-night refreshment, and campsite welfare activity operating under the event licence and approved operating schedule.',
  ...
  risk_assessment_methodology:
    'The operational risk assessment is derived from the event profile, route and capacity analysis, deployment model, emergency procedures, and the selected annexes that define the KSS scope. Each activity or position is reviewed against the likely hazard, the persons who may be harmed, the controls already built into this CMP, and the expected residual position once those controls are applied and supervised live by Event Control.',
  ...
  additional_operational_risks: lines(
    ...
    'Late-night campsite welfare demand - Campers, welfare staff, and overnight patrols - Maintain overnight patrol schedule, quiet-hours intervention, welfare escalation, and direct reporting to the campsite supervisor and control.'
  ),
  ...
  emergency_search_zones:
    'Emergency search zones are pre-identified around the main stage fan, west service lane, east ingress plaza, VIP or backstage compound, and campsite welfare hub. Sterile routes to RV1 and the south boulevard are protected immediately following any suspected device report or evacuation instruction, with supervisors assigned to maintain exclusion until police or emergency command authorises release.',
  ...
  site_maps_and_route_diagrams: lines(
    ...
    'VIP and backstage controlled-area map',
