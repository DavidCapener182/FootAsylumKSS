export type CmpMasterTemplateCategory = 'Plans' | 'Checklists' | 'Logs' | 'Briefings'

export type CmpMasterTemplateOrientation = 'landscape' | 'portrait'

export type CmpMasterTemplateTone = 'neutral' | 'positive' | 'negative'

export type CmpMasterTemplateNoticeTone = 'info' | 'warning' | 'danger'

export type CmpMasterTemplateTitleTone = 'default' | 'warning' | 'danger'

export type CmpMasterTemplateIconKey =
  | 'shield'
  | 'radio'
  | 'users'
  | 'list'
...
export type CmpMasterTemplateNotice = {
  tone: CmpMasterTemplateNoticeTone
  title?: string
  body: string | string[]
}

type CmpMasterTemplateBase = {
  id: string
  documentCode: string
  order: number
  category: CmpMasterTemplateCategory
  icon: CmpMasterTemplateIconKey
  title: string
  description: string
  filename: string
  orientation: CmpMasterTemplateOrientation
  notice?: CmpMasterTemplateNotice
  titleTone?: CmpMasterTemplateTitleTone
}

export type CmpMasterTemplateField = {
  label: string
}

export type CmpMasterTemplateTableColumn = {
  key: string
  label: string
  width: string
  align?: 'left' | 'center'
  tone?: CmpMasterTemplateTone
}

export type CmpMasterTemplateTable = CmpMasterTemplateBase & {
  kind: 'table'
  infoFields: CmpMasterTemplateField[]
  columns: CmpMasterTemplateTableColumn[]
  emptyRows: number
  footerNote?: string
  footerRight?: string
...
export type CmpMasterTemplateIncidentForm = CmpMasterTemplateBase & {
  kind: 'incident_form'
  infoRows: Array<[string, string]>
  categories: string[]
}

export type CmpMasterTemplateNarrativeSection =
  | {
      type: 'textbox'
      title: string
      heightClass: string
...
export type CmpMasterTemplateNarrativeForm = CmpMasterTemplateBase & {
  kind: 'narrative_form'
  headerFields: CmpMasterTemplateField[]
  sections: CmpMasterTemplateNarrativeSection[]
}

export type CmpMasterTemplateEmergencyActionPlan = CmpMasterTemplateBase & {
...
export type CmpMasterTemplateSuspiciousItemReport = CmpMasterTemplateBase & {
  kind: 'suspicious_item_report'
  infoRows: Array<[string, string]>
  hotQuestions: string[]
}

export type CmpMasterTemplateDefinition =
  | CmpMasterTemplateTable
  | CmpMasterTemplateIncidentForm
  | CmpMasterTemplateNarrativeForm
  | CmpMasterTemplateEmergencyActionPlan
  | CmpMasterTemplateSuspiciousItemReport

export const CMP_MASTER_TEMPLATE_CATEGORIES: CmpMasterTemplateCategory[] = [
  'Plans',
  'Checklists',
  'Logs',
  'Briefings',
...
export const CMP_MASTER_TEMPLATES: CmpMasterTemplateDefinition[] = [
  {
    id: 'uniform-ppe-allocation-log',
    documentCode: 'CMP-MT-01',
    order: 1,
    category: 'Checklists',
    icon: 'shield',
  ...
  },
  {
    ...
    documentCode: 'CMP-MT-03',
    order: 3,
    category: 'Checklists',
    icon: 'users',
    title: 'Staff Sign-in / Sign-out Sheet',
    description: 'Attendance control sheet for deployed staff, agencies, roles, and final sign-off.',
...
export function getCmpMasterTemplateById(templateId: string | null | undefined) {
  const normalizedId = String(templateId || '').trim().toLowerCase()
  return CMP_MASTER_TEMPLATES.find((template) => template.id === normalizedId) ?? null
}

export function groupCmpMasterTemplatesByCategory() {
  return CMP_MASTER_TEMPLATE_CATEGORIES.map((category) => ({
    category,
    templates: CMP_MASTER_TEMPLATES.filter((template) => template.category === category),
  }))
}
