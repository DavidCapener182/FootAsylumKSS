...
export const EMP_GUIDED_GROUPS: EmpGuidedGroup[] = [
  {
    ...
    questions: [
      ...
      { key: 'event_type', fieldKey: 'event_type', label: 'Event type', type: 'text', placeholder: 'Outdoor concert, festival, licensed event' },
  ...
  },
  {
    ...
    questions: [
      ...
      { key: 'has_camping', label: 'Overnight bar asset protection applies', type: 'checkbox' },
      { key: 'has_vip_backstage', label: 'Restricted compound / accreditation applies', type: 'checkbox' },
      { key: 'has_front_of_stage', label: 'High-demand bar queue area applies', type: 'checkbox' },
      { key: 'has_traffic_routes', label: 'Service route or pedestrian interface applies', type: 'checkbox' },
      { key: 'has_search_screening', label: 'Dedicated search or screening applies', type: 'checkbox' },
      { key: 'has_stewarding_deployment', label: 'Include stewarding / queue marshal deployment annex', type: 'checkbox' },
      { key: 'has_emergency_action_cards', label: 'Include emergency action cards annex', type: 'checkbox' },
      { key: 'applicable_areas', label: 'KSS areas / zones covered by this plan', type: 'textarea', placeholder: 'Arena\nIngress routes\nEgress routes\nWelfare hub' },
  ...
  },
  {
    ...
    questions: [
      ...
      { key: 'camper_count', label: 'Expected overnight bar asset count, if applicable', type: 'number' },
      { key: 'audience_age_profile', fieldKey: 'audience_age_profile', label: 'Audience age profile', type: 'text' },
      { key: 'crowd_behaviour_level', label: 'Behaviour / queue pressure level', type: 'select', options: riskLevelOptions },
      { key: 'crowd_type', label: 'Audience or bar behaviour notes', type: 'textarea', help: 'Optional. Leave blank to use standard wording for the selected level.' },
      ...
      { key: 'peak_periods', fieldKey: 'peak_periods', label: 'Expected KSS peak periods', type: 'textarea', placeholder: '17:00 to 21:00 - bar service peak' },
    ],
  },
  {
    key: 'site_design',
    ...
    questions: [
      { key: 'entrances', label: 'KSS access points / screening locations', type: 'textarea' },
      { key: 'exits', label: 'Close-down / handover routes', type: 'textarea' },
      { key: 'key_zones', fieldKey: 'key_zones', label: 'Key zones and operational areas', type: 'textarea' },
      { key: 'controlled_areas', fieldKey: 'controlled_areas', label: 'Allocated or restricted areas', type: 'textarea' },
      ...
      { key: 'primary_routes', label: 'Primary service, queue, or pedestrian interface routes', type: 'textarea' },
      ...
      { key: 'movement_pressure_points', label: 'Bar or service pressure points', type: 'textarea' },
  ...
  },
  {
    ...
    questions: [
      { key: 'gross_area', fieldKey: 'gross_area', label: 'Gross KSS-allocated area', type: 'text' },
      { key: 'net_area', fieldKey: 'net_area', label: 'Net usable queue / service area', type: 'text' },
      ...
      { key: 'egress_route_count', label: 'Number of close-down / clear-down routes', type: 'number' },
  ...
  },
  {
    ...
    questions: [
      { key: 'public_ingress_time', fieldKey: 'public_ingress_time', label: 'KSS public-facing opening time', type: 'text' },
      { key: 'operational_hours', fieldKey: 'operational_hours', label: 'Operational hours by phase', type: 'textarea' },
      { key: 'ingress_routes_holding_areas', fieldKey: 'ingress_routes_holding_areas', label: 'Queue areas and access points', type: 'textarea' },
      { key: 'admission_search_posture', label: 'Event search interface', type: 'select', options: searchPostureOptions },
      ...
      { key: 'high_density_risk_level', label: 'High-demand bar risk level', type: 'select', options: riskLevelOptions },
      { key: 'internal_queue_risk_level', label: 'Internal queue risk level', type: 'select', options: riskLevelOptions },
      { key: 'transport_interface', fieldKey: 'transport_interface', label: 'Bar close-down / egress interface', type: 'textarea' },
      { key: 'dispersal_routes', fieldKey: 'dispersal_routes', label: 'Close-down / stock movement routes', type: 'textarea' },
...
export function generateEmpFieldValuesFromGuidedAnswers(
  ...
  values.overspill_controls = `Overspill and surge risk is assessed as ${levelLabel(answer(answers, 'overspill_risk_level'))}. Supervisors should monitor bar queue length, service-lane obstruction, neighbouring land use, welfare demand, refusal conflict, and escalation thresholds through Event Control.`
  values.circulation_controls = `Circulation risk is assessed as ${levelLabel(answer(answers, 'circulation_risk_level'))}. Controls should protect principal pedestrian routes, accessible routes, emergency corridors, stock routes, service crossings, and known dwell points around KSS areas.`
  values.high_density_controls = `High-demand bar risk is assessed as ${levelLabel(answer(answers, 'high_density_risk_level'))}. Controls should include observation, supervisor escalation, holding inflow where needed, response-team positioning, and medical or welfare interface.`
  values.internal_queue_controls = `Internal queue risk is assessed as ${levelLabel(answer(answers, 'internal_queue_risk_level'))}. Queues for bars, stores, welfare, toilets, concessions, or attractions must be kept off principal circulation, service lanes, and emergency routes.`
