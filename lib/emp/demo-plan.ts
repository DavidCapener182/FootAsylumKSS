...
export const EMP_DEMO_PLAN_VALUES: Record<string, string> = {
  ...
  alcohol_profile:
    'Alcohol sales are expected to create the highest behavioural demand between 17:00 and 22:30. Intoxication risk is highest in bar queue areas, bar close and activation peak periods, and staff campsite returns after headline performances. Refusals and intervention logs are expected to increase after 20:00 and must be monitored by control and supervisors.',
  camping_profile:
  ...
  peak_periods: lines(
    ...
    '22:30 to 00:30 - phased egress, taxi rank demand, shuttle loading, and staff campsite return.'
  ),
  ...
  dim_aliced_location:
    'Last-mile factors include rail and shuttle interfaces, the temporary taxi rank, east car park pedestrian route, residential streets to the south, the public park boundary to the west, lighting, weather exposure and grassed links between the arena and staff campsite.',
  dim_aliced_ingress:
  ...
  dim_aliced_dynamics:
    'Dispersal considerations include phased departure from the arena, staff campsite return, taxi and shuttle demand, route lighting, welfare support, and maintaining separation between public exit routes, staff routes and service movements after show close.',
  ...
  ramp_arrival:
    'Static and dynamic gathering spaces include the east holding plaza, west bar village, main arena spine, welfare hub, accessible viewing area, staff campsite crossover, taxi-rank interface and food court dwell areas. Each area requires a defined footprint, holding capacity, clear route ownership and supervisor review during peak times.',
  ramp_movement:
    'Peak movement demand is forecast around the main bar operation, bar village, welfare hub, accessible platform, and staff campsite crossover. Counterflow is most likely between the main arena exit and the west service and bar zone after headline performances and therefore requires active steward intervention.',
  ramp_profile:
  ...
  relief_and_contingency:
    'Relief staff are assigned by supervisor to maintain statutory breaks without leaving fixed posts uncovered. A reserve of 12 trained staff is held each day for sickness, queue congestion, adverse weather, and emergency redeployment. Overnight staff campsite cover includes a relief handover and fatigue check.',
  escalation_staffing:
  ...
  dynamic_escalation_triggers: lines(
    ...
    'Visible compression, stop-start movement, barrier loading, or customer distress is reported by supervisors.',
  ...
  high_density_controls:
    'The west bar village is monitored from bar supervisor, activation supervisor, and roaming supervisor positions. Threshold triggers include visible stop-start movement, compression at the front barrier, or lateral movement toward route edges. Control options include public messaging, area soft-holds, lateral redistribution, reinforcement, and client liaison through production.',
  ...
  risk_assessment_source_notes:
    'The supporting operational risk assessment identifies crowd density at the west bar village, bag-search delay, bar queue spillback, staff campsite crossover, late-night taxi demand, degraded grass routes in wet weather, and safeguarding demand linked to intoxication and fatigue. These trigger points have been carried into the EMP controls, staffing peaks, route-protection measures, and emergency decision thresholds.',
