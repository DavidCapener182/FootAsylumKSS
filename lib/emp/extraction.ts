import type { EmpDocumentKind } from '@/lib/emp/master-template'

export interface EmpSourceDocumentForExtraction {
  id: string
  document_kind: EmpDocumentKind | string
  file_name: string
  extracted_text: string | null
}

export interface EmpFieldCandidate {
  valueText: string
...
function firstScalarMatch(
  documents: EmpSourceDocumentForExtraction[],
  patterns: RegExp[]
): EmpFieldCandidate | null {
...
function derivePlanTitle(eventName: string | undefined | null) {
  ...
  return `${cleaned} - Event Management Plan`
}

function groupDocuments(documents: EmpSourceDocumentForExtraction[]) {
...
function withFallback(
  primary: EmpFieldCandidate | null,
  fallback: EmpFieldCandidate | null
): EmpFieldCandidate | null {
  return primary || fallback
}

export function deriveEmpFieldCandidates(documents: EmpSourceDocumentForExtraction[]) {
  const grouped = groupDocuments(documents)
  const candidates: Record<string, EmpFieldCandidate> = {}
  ...
  const scalarMappings: Record<string, EmpFieldCandidate | null> = {
  ...
  const sectionMappings: Record<string, EmpFieldCandidate | null> = {
    ...
    dim_aliced_activity: firstSectionMatch(grouped.all, ['Arrival', 'Activity']),
    dim_aliced_location: firstSectionMatch(grouped.all, ['Last Mile', 'Location']),
    ...
    dim_aliced_dynamics: firstSectionMatch(grouped.all, ['Dispersal', 'Dynamics']),
    ramp_routes: firstSectionMatch(grouped.all, ['ROUTES']),
    ramp_arrival: firstSectionMatch(grouped.all, ['AREAS', 'ARRIVAL']),
    ...
    queue_design: firstSectionMatch(grouped.operations, [
      'Queuing Systems',
      'Queuing Systems (Disney-Style & Adaptive for Bar Areas)',
      'Disney-Style Queue Management System',
      'Queue Management Philosophy',
      'Adaptive Queue Design',
    ]),
    overspill_controls: firstSectionMatch(grouped.operations, ['Queue Management for High-Demand Bars', 'Adaptive Queue Design', 'Queuing Systems']),
    ...
    refusal_false_id_protocol: firstSectionMatch(grouped.operations, [
      'Refusals - Challenge 21 and Licensing Compliance',
      'Refusals – Challenge 21 and Licensing Compliance',
      'Refusal & False ID Protocols',
      '14.1 Refusals',
    ]),
    ejection_protocol: firstSectionMatch(grouped.operations, ['Ejection Protocol', '14.2 Ejection Protocol']),
    confiscation_process: firstSectionMatch(grouped.operations, ['Confiscation Process', '14.3 Confiscation Process']),
    ejection_safeguarding: firstSectionMatch(grouped.operations, ['Safeguarding within Ejections', '14.4 Safeguarding within Ejections']),
    ...
    close_down_operations: firstSectionMatch(grouped.operations, [
      'Post-Event Close Down and Debrief Procedures',
      'Close Down Operations',
      '19.1 Objectives of Close Down Operations',
    ]),
    end_of_shift_reporting: firstSectionMatch(grouped.operations, ['End-of-Shift and End-of-Event Reporting', '19.2 End-of-Shift and End-of-Event Reporting']),
    asset_security_demobilisation: firstSectionMatch(grouped.operations, ['Asset Security and Demobilisation', '19.4 Asset Security and Demobilisation']),
    health_safety_overview: firstSectionMatch(grouped.operations, [
      'Health & Safety and Risk Assessment Overview',
      'Health and Safety and Risk Assessment Overview',
      '22.0 Health & Safety and Risk Assessment Overview',
      'Core Health & Safety Principles',
    ]),
    ...
    version_history_summary: firstSectionMatch(grouped.operations, ['Version History Summary', '21.1 Version History Summary']),
    contact_directory: firstSectionMatch(grouped.operations, ['Contact Directory', '21.2 Contact Directory', 'Key Contacts']),
  }
