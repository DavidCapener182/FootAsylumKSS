-- Preserve every visible audit/FRA reference before a newer report replaces it.
create table public.fa_store_audit_history (
 id uuid primary key default gen_random_uuid(), store_id uuid not null references public.fa_stores(id),
 kind text not null check(kind in ('H&S','FRA')), audit_year integer not null,
 audit_number integer, visit_date date not null, percentage numeric,
 pdf_path text, captured_at timestamptz not null default now(),
 unique nulls not distinct(store_id,kind,audit_number,visit_date,pdf_path)
);
create index on public.fa_store_audit_history(store_id,visit_date desc);
alter table public.fa_store_audit_history enable row level security;
revoke all on public.fa_store_audit_history from anon,authenticated;
grant all on public.fa_store_audit_history to service_role;

create table public.fa_audit_studio_publications (
 audit_id uuid primary key references public.fa_audit_studio_audits(id),
 store_id uuid not null references public.fa_stores(id), audit_year integer not null,
 audit_number integer not null check(audit_number between 1 and 3),
 pdf_path text not null, pdf_sha256 text not null,
 published_by uuid not null references auth.users(id), published_at timestamptz not null default now()
);
create index on public.fa_audit_studio_publications(store_id,audit_year,audit_number);
alter table public.fa_audit_studio_publications enable row level security;
revoke all on public.fa_audit_studio_publications from anon,authenticated;
grant all on public.fa_audit_studio_publications to service_role;

-- Snapshot existing references without copying PDF contents into the database.
insert into public.fa_store_audit_history(store_id,kind,audit_year,audit_number,visit_date,percentage,pdf_path)
select id,'H&S',extract(year from d),n,d,pct,p from public.fa_stores s cross join lateral (
 values(1,s.compliance_audit_1_date,s.compliance_audit_1_overall_pct,s.compliance_audit_1_pdf_path),
 (2,s.compliance_audit_2_date,s.compliance_audit_2_overall_pct,s.compliance_audit_2_pdf_path),
 (3,s.compliance_audit_3_date,s.compliance_audit_3_overall_pct,null::text)
) x(n,d,pct,p) where d is not null on conflict do nothing;
insert into public.fa_store_audit_history(store_id,kind,audit_year,visit_date,percentage,pdf_path)
select id,'FRA',extract(year from fire_risk_assessment_date),fire_risk_assessment_date,fire_risk_assessment_pct,fire_risk_assessment_pdf_path
from public.fa_stores where fire_risk_assessment_date is not null on conflict do nothing;
-- Third visits can now retain a downloadable report as well.
alter table public.fa_stores add column if not exists compliance_audit_3_pdf_path text;

create function public.fa_publish_studio_audit(p_audit uuid,p_store uuid,p_user uuid,p_path text,p_hash text)
returns public.fa_audit_studio_publications language plpgsql security invoker set search_path='' as $$
...
begin
 ...
 if parent_number is not null then
  ...
  select coalesce(max(k),0)+1 into n from generate_series(1,3) k where extract(year from (vals->>('compliance_audit_'||k||'_date'))::date)=yr;
 end if;
...
grant execute on function public.fa_publish_studio_audit(uuid,uuid,uuid,text,text) to service_role;
