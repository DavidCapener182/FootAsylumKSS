...
export function ComplianceProgressPanel({ data }: { data: DashboardData }) {
  ...
  const firstAuditsRequired = safeNumber(data.auditStats?.firstAuditsRequired, totalStores)
  const firstAuditsNotRequired = safeNumber(data.auditStats?.firstAuditsNotRequired)
  ...
  return (
    <Panel title="Compliance Progress" icon={Activity} actionLabel={`${totalStores} stores`}>
      <div className="space-y-2.5 sm:space-y-4">
        <ComplianceProgressRow label="Audit 1 Complete" value={audit1Complete} percentage={safeNumber(data.auditStats?.firstAuditPercentage, percent(audit1Complete, firstAuditsRequired))} tone="success" />
        {firstAuditsNotRequired > 0 && <p className="text-xs text-slate-500">{firstAuditsNotRequired} new stores excluded from Audit 1 requirements.</p>}
