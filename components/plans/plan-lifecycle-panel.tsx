...
export function PlanLifecyclePanel({ planType, initialData, completedRequired, totalRequired }: { planType: PlanType; initialData: PlanLifecycleData; completedRequired: number; totalRequired: number }) {
  ...
  return <Card className="workspace-plan-control mb-4 border-slate-200 shadow-sm"><details><summary className="flex min-h-[52px] cursor-pointer list-none flex-wrap items-center justify-between gap-2 px-4 py-3"><span className="flex items-center gap-2 text-sm font-semibold"><GitCompareArrows className="h-4 w-4" />Plan control <span className="text-xs font-normal text-slate-500">{completedRequired}/{totalRequired} required fields</span></span><span className="rounded-full bg-slate-950 px-3 py-1 text-xs font-bold uppercase text-lime-300">v{data.plan.version_number} · {data.plan.status} <span aria-hidden="true">⌄</span></span></summary><CardContent className="grid gap-4 lg:grid-cols-[1.2fr_1fr]">
