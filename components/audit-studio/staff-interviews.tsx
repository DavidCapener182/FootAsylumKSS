...
export function StaffInterviews({ doc, template, readOnly, open, onOpenChange, update, onQuestion, status, section }: {
  ...
  onQuestion: (id: string) => void; status: string; section: number;
}) {
  const [selected, setSelected] = useState(0);
  const [remove, setRemove] = useState(false);
  const [topics, setTopics] = useState("section");
  useEffect(() => { if (open) setTopics("section"); }, [open, section]);
  ...
  const answer = (id: string, ask: string, patch: Partial<StaffInterviewAnswer>) => changeStaff(s => ({
    ...s, answers: { ...s.answers, [id]: { ...(s.answers[id] || { ...emptyInterviewAnswer(), asked: ask, sampledRisk: id === "risks", ...(practicalDefinition(id) ? {practical:{...newPracticalCheck(), optionalSampling: true}} : {}) }), ...patch } },
  }));
  const add = () => {
    const id = clientId();
      ...
      <SheetContent className="flex flex-col bg-[#f5f6f1] pt-[env(safe-area-inset-top)] pb-[env(safe-area-inset-bottom)] text-[#17291f] sm:w-[640px] sm:max-w-[90vw]">
        ...
        <div className="min-h-0 flex-1 overflow-y-auto overscroll-contain p-4 sm:p-5">
          ...
          {current && <>
            ...
            <label className="mb-4 block text-sm font-semibold">Show topics<select className={field} value={topics} onChange={e => setTopics(e.target.value)}><option value="section">This section</option><option value="all">All topics and answers</option><option value="recorded">Recorded for this colleague</option></select></label>
            ...
            <div className="space-y-3">
              {prompts.filter(p => topics === "all" || (topics === "section" ? Number(p.questionId.split(".")[0]) === section || (section === 5 && current.answers[p.id]?.sampledRisk) : !!current.answers[p.id])).map(p => {
                ...
                return <details key={`${current.id}-${p.id}`} className="rounded-xl border border-slate-200 bg-white">
                  ...
                  <div className="space-y-4 border-t border-slate-100 p-4">
                    ...
                    {!a && <button className={button} disabled={readOnly} onClick={() => answer(p.id, p.ask, {})}>Ask this topic</button>}
                    {a && <>
                    ...
                    <label className="block text-sm font-semibold">{!practicalActive && assessment === "not-applicable" ? "Why does this not apply? (required)" : "Outcome / explanation"}<GrowingTextarea rows={2} className={field} value={a?.outcome || ""} disabled={readOnly} onChange={e => answer(p.id,p.ask,{outcome:e.target.value})} /></label>
                    ...
                    {!readOnly && a && <button className="min-h-11 text-xs text-red-700 underline" onClick={() => { if (window.confirm("Clear this recorded answer? The linked audit result will be recalculated.")) changeStaff(s => {const answers = {...s.answers}; delete answers[p.id]; return {...s,answers};}); }}>Reset to Not asked</button>}
                    </>}
                  </div>
                </details>;
              })}
