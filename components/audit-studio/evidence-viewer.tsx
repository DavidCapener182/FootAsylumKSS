...
export function EvidenceViewer({open, onOpenChange, url, file, label}: {
  open: boolean; onOpenChange: (open: boolean) => void; url: string; file?: Blob; label: string;
}) {
  ...
  return <Sheet open={open} onOpenChange={onOpenChange}>
    <SheetContent className="flex flex-col bg-white pt-[max(1rem,env(safe-area-inset-top))] pb-[max(1rem,env(safe-area-inset-bottom))] sm:w-[900px] sm:max-w-[95vw]">
      <header className="shrink-0 border-b p-4 pr-14"><SheetTitle>Audit evidence</SheetTitle><SheetDescription className="mt-2 break-words">{label}</SheetDescription></header>
      ...
      <footer className="shrink-0 border-t p-3"><SheetClose asChild><button type="button" className="min-h-12 w-full rounded-lg border border-slate-300 bg-white px-4 font-semibold">Back to audit</button></SheetClose></footer>
    </SheetContent>
  </Sheet>;
}
